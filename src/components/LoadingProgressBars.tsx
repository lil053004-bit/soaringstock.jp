...
export default function LoadingProgressBars({ isVisible }: LoadingProgressBarsProps) {
  ...
  const stages = [
    { label: 'データ収集中', progress: stage1, color: '#2563EB' },
    { label: 'データ処理中', progress: stage2, color: '#1D4ED8' },
    { label: '情報集約中', progress: stage3, color: '#1E40AF' },
  ];
      ...
      {stages.map((stage, index) => (
        <div key={index} className="space-y-1">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-charcoal-light">{Math.floor(stage.progress)}%</span>
          </div>
          <div className="relative w-full h-2 bg-beige-dark rounded-full overflow-hidden border border-yellow-primary/30">
            <div
